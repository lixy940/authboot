...
public class RegexUtils {
    ...
    private final static String MOBILE_REGEX = "^(13[0-9]|15[012356789]|17[3678]|18[0-9]|14[57])[0-9]{8}$";

    //验证手机号以1开头，总11位
    private final static String MOBILE_NUM_REGEX = "^([1-9]{1})(\\d{10})$";
    //验证身份证位数15或18
    private final static String ID_CARD_REGEX = "^\\d{15}$|^\\d{17}([0-9]|x|X)$";
    //车牌号正则表达式
    private final static String CAR_NUM_REGEX = "^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[警京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{0,1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$";

    private final static String EMAIL_REGEX = "^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$";

    private final static String BANK_NO_REGEX = "^([1-9]{1})(\\d{11,18})$";

    private final static String ALL_NUM_REGEX = "^[0-9]+$";
    //两位小数有效位的数字校验
    private final static String TWO_DECIMAL_REGEX = "(^[1-9](\\d+)?(\\.\\d{1,2})?$)|(^(0){1}$)|(^\\d\\.\\d{1,2}?$)";
    //验证8为以内的整数
    private final static String INTEGER_NUM_8_REGEX = "(^[1-9]{1}([0-9]{1,7})?$)|(^(0){1}$)";
    //验证8位以内的整数 不包含0
    private final static String INTEGER_NUM_8_Ex0_REGEX = "^[1-9]{1}([0-9]{1,7})?$";
    //验证全是float
    private final static String FLOAT_REGEX = "^[0-9]+([.]{1}[0-9]+){0,1}$";
    //日期格式，2014-01-01或2014-01-01 12:00:00
    private final static String DATE_REGEX = "(^[1-9]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$)|(^[1-9]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])\\s+(20|21|22|23|[0-1]\\d):[0-5]\\d:[0-5]\\d$)";
    //验证日期后的格式为2014-01-01 15:05:29.0
    private final static String TIMESTAMP_REGEX = "^[1-9]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])\\s+(20|21|22|23|[0-1]\\d):[0-5]\\d:[0-5]\\d(\\.\\d)$";

    //中英文/数字及_
    private final static String SPECIAL_CHAR_REGEX = "[`~!@#$%^&*+=|{}':;',/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
    ...
    public static boolean validateSpecialChar(String str) {
        return validate(str, SPECIAL_CHAR_REGEX);
    }
...
//        Optional.of(validateSpecialChar("333")).ifPresent(System.out::println);
        System.out.println(validateSpecialChar("你好34_d ~f"));
    }
}
